# consumers/trade_raw_archiver.py
"""
Market Trade Raw Archiver (Cold Path)
─────────────────────────────────────
Kafka topic `market.trade` → Toàn bộ boards → Parquet → MinIO.

Lưu trữ dữ liệu thô (tất cả board_id) vào MinIO dưới dạng Parquet,
phân vùng theo ngày để phục vụ batch analytics và ML.

Cấu trúc file trên MinIO:
  raw/market_trade/date=2026-04-20/103000_103500.parquet
  (mỗi file chứa data trong khoảng 5 phút)
"""
...
CONSUMER_GROUP      = "minio-trade-archiver-v1"
...
def _flush_to_minio(minio_client, consumer, buffer: list, flush_start: str):
    ...
    # để tránh sai partition khi flush qua nửa đêm UTC
        ...
        while running:
            ...
            if should_flush:
                for attempt in range(MAX_RETRY):
                    ...
                    # Kafka sẽ re-deliver khi restart
